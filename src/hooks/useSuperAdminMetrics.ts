...
export function useSuperAdminMetrics(filters: SuperAdminFilters) {
  return useQuery({
    ...
    queryFn: async (): Promise<SuperAdminMetrics> => {
      ...
      // Get total products
      let productsQuery = supabase.from('products').select('id, clinic_id').range(0, 999999); // Increased range
      ...
      let bookingsQuery = supabase.from('bookings').select('id, clinic_id, created_at').range(0, 999999); // Increased range
...
//       console.log('Super admin leads data:', leadsCount || 0, 'leads found');
      ...
      let salesQuery = supabase.from('sales').select('amount, clinic_id, created_at').range(0, 999999); // Increased range
      if (filters.clinicIds.length > 0) {
      ...
      // Get costs data
      let costsQuery = supabase.from('costs').select('amount, clinic_id, created_at').range(0, 999999); // Increased range
      if (filters.clinicIds.length > 0) {
